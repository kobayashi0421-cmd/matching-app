...
export default function ProfilePage() {
  ...
  const handleDeleteAccount = async () => {
    ...
    const confirm2 = window.confirm(
      '【最終確認】アカウントを削除すると、マッチング結果、チャット履歴、プロフィール等の全データが完全に消去され元に戻せません。実行しますか？'
    )
    if (!confirm2) return
    ...
    try {
      ...
      if (!session) {
        throw new Error('セッション情報が取得できませんでした。再ログインしてお試しください。')
      }
            ...
            <div className="profile-content-scroll">
              ...
              >
                <p style={{ fontSize: '12px', color: '#666', marginBottom: '12px' }}>
                  アカウントを削除すると、メッセージ履歴等の全てのデータが消去されます。
                </p>
                <button
                  type="button"
